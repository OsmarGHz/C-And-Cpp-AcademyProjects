...
typedef struct nodoalt{
    int nLista, dato;
    struct nodoalt *siguiente;
} NODOALT;
...
NODOALT *crearNodoALT(){
    NODOALT *nuevo;
    nuevo = (NODOALT *) malloc(sizeof(NODOALT));
    if(nuevo != NULL){
        nuevo->siguiente=NULL;
    }
    return nuevo;
...
// Esta seccion abarca nuevas funciones de la actividad presente

void insertarInfoNodoALT(int nLista, int dato, NODOALT **nuevo){
    (*nuevo)->nLista = nLista;
    (*nuevo)->dato = dato;
}

void agregadoDeNodoALT(int nLista, int numero, NODOALT **nodoRaiz){
    int errores = 0;
    NODOALT *nuevo, *aux;
    if (*nodoRaiz == NULL){
        nuevo = crearNodoALT();
...
void eliminadorDeNodoALT(int nPosicion, NODOALT **nodoRaizRemitente, NODOALT **nodoRaizDestinatario){
    NODOALT *aux, *anterior;
    int errores = 0;

        aux=*nodoRaizRemitente;
        if ((*nodoRaizRemitente)->nLista == nPosicion){
            *nodoRaizRemitente = (*nodoRaizRemitente)->siguiente;
            //alumnoAEliminarEncontrado(nListaTemp, alu, calif, &aux);
            agregadoDeNodoALT(aux->nLista,aux->dato,nodoRaizDestinatario);
            free(aux);
        }else{
            aux=*nodoRaizRemitente;
            anterior=*nodoRaizRemitente;
            while (aux != NULL){
                if (aux->nLista == nPosicion) break;
                anterior = aux;
                aux = aux->siguiente;
            }
            if (aux == NULL){
...
int validarNumeroListaALT(int numeroLista, int rangoMaximo) {
    // Comprobamos si el numero de lista está en el rango permitido
    if (numeroLista < 1 || numeroLista > rangoMaximo) {
...
void lectorDePosiciones(int nLista, NODOALT **nodoRaizRemitente, NODOALT **nodoRaizDestinatario, int rangoMaximo){
    int nPosicion, errores;
    printf("Ingrese el numero de posicion, del 1 al %d (estamos en lista %d): ", rangoMaximo, nLista);
    scanf(" %d", &nPosicion);
    errores = validarNumeroListaALT(nPosicion, rangoMaximo); // Llamamos a la función validarNumeroLista
    limpiarBuffer();
    if (errores) nPosicion=rangoMaximo;
    eliminadorDeNodoALT(nPosicion, nodoRaizRemitente, nodoRaizDestinatario);
    rangoMaximo--;    
}

void imprimir(int numeroListaLig, NODOALT **nodoRaiz){
    NODOALT *aux;
    aux=*nodoRaiz;
    printf("\nResultados lista %d:\n", numeroListaLig);
    ...
    do{
        ...
        printf("Numero de posicion: %d, Valor: %d\n", aux->nLista,aux->dato);
        aux=aux->siguiente;
    } while (aux!=NULL);
...
int main(){
    ...
    */
    NODOALT *nodoRaizAlt = NULL, *nodoRaizAlt2 = NULL, *nodoRaizAlt3 = NULL;
    int i=0, nodosExitosos=0;
    agregadoDeNodoALT(1,67,&nodoRaizAlt);
    agregadoDeNodoALT(2,45,&nodoRaizAlt);
    agregadoDeNodoALT(3,100,&nodoRaizAlt);
    agregadoDeNodoALT(4,98,&nodoRaizAlt);
    agregadoDeNodoALT(5,453,&nodoRaizAlt);
    agregadoDeNodoALT(6,234,&nodoRaizAlt);
    agregadoDeNodoALT(1,129,&nodoRaizAlt2);
    agregadoDeNodoALT(2,23,&nodoRaizAlt2);
    agregadoDeNodoALT(3,65,&nodoRaizAlt2);
    agregadoDeNodoALT(4,12,&nodoRaizAlt2);
    agregadoDeNodoALT(5,7,&nodoRaizAlt2);
    agregadoDeNodoALT(6,1,&nodoRaizAlt2);
    agregadoDeNodoALT(7,0,&nodoRaizAlt2);
    
    for (i = 0; i < 3; i++){
        lectorDePosiciones(1,&nodoRaizAlt, &nodoRaizAlt3, 6);
    }
    for (i = 0; i < 2; i++){
        lectorDePosiciones(2,&nodoRaizAlt2, &nodoRaizAlt3, 7);
    }
    imprimir(1, &nodoRaizAlt);
    imprimir(2, &nodoRaizAlt2);
    imprimir(3, &nodoRaizAlt3);
