#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h> 
#include <time.h>
#include <malloc.h>
#define N 4

/*
Integrantes del equipo:
++ Hernandez Prado Osmar Javier
++ Lopez Momox Limhi Gerson
*/

typedef struct{
    int hora, minuto;
} TIMESTAMP;

typedef struct alumno{
    char nombreComp [60];
    char matricula [10];
    int minutosTardados;
} ALUMNO;


typedef struct nodo{
    int nLista;
    TIMESTAMP marcaDeTiempo;
    struct nodo *siguiente;
} NODO;

NODO *crearNodo(int nListaTemp, int horaTemp, int minutoTemp){
    NODO *nuevo = NULL;
    nuevo = (NODO *) malloc(sizeof(NODO));
    if(nuevo != NULL){
        nuevo->nLista = nListaTemp;
        nuevo->marcaDeTiempo.hora = horaTemp;
        nuevo->marcaDeTiempo.minuto = minutoTemp;
        nuevo->siguiente = NULL;
    }
    return nuevo;
}

void limpiarBuffer(){
    char c;
    while ((c = getchar()) != '\n' && c != EOF);
}

int validarNombre(const char nombre[], int *numeroNombre, int *numeroApellido) {
    int i, palabraValida = 0, letrasPalabra = 0, primerNombreListo=0, primerApellidoListo=0;

    for (i = 0; nombre[i] != '\0'; i++) {
        // Si encontramos un espacio, reiniciamos el contador de letras de la palabra
        if (isspace(nombre[i])) {
            palabraValida = 0;
            letrasPalabra = 0;
            continue;
        }

        // Comprobamos que el primer caracter de una palabra sea ser mayúscula
        if (!palabraValida) {
            if (!isupper(nombre[i])) {
                printf("\n\t\t * El nombre debe comenzar con mayuscula *\n");
                return 1; // Codigo de error
            }
            if (primerNombreListo==0 && primerApellidoListo == 0){
                *numeroNombre = nombre[i];
                primerNombreListo++;
            }else if(primerApellidoListo == 0){
                *numeroApellido = nombre[i];
                primerApellidoListo++;
            }
            
            palabraValida = 1;
        } else {
            // Si es una letra, debe ser minúscula
            if (!islower(nombre[i])) {
                printf("\n\t\t * Las letras del nombre deben estar en minuscula *\n");
                return 1; // Codigo de error
            }
        }

        letrasPalabra++;

        // Comprobamos si la palabra tiene al menos dos caracteres
        if (isspace(nombre[i + 1]) && letrasPalabra < 2) {
            printf("\n\t\t * Cada palabra del nombre debe tener al menos dos caracteres *\n");
            return 1; // Codigo de error
        }
    }
    return 0; // Sin errores
}

int validarMatricula(const char matricula[]) {
    int i, errors = 0;

    // Comprobamos si la longitud de la matricula es exactamente 10 caracteres
    if (strlen(matricula) != 10) {
        printf("\n\t\t * La matricula debe tener exactamente 10 digitos *\n");
        errors++;
    }else{
        // Comprobamos que todos los caracteres sean digitos
        for (i = 0; i < 10; i++) {
            if (!isdigit(matricula[i])) {
                printf("\n\t\t * La matricula debe contener solo digitos *\n");
                errors++;
                break;
            }
        }
    }
    return errors;
}

int validarMatriculaRepetida(const char matricula[], const struct alumno alu[], int index) {
    int i;

    for (i = 0; i < index; i++) {
        if (strcmp(alu[i].matricula, matricula) == 0) {
            printf("\n\t\t * La matricula ingresada ya existe para otro alumno *\n");
            return 1; // Matrícula repetida encontrada
        }
    }
    return 0; // No se encontraron matrículas repetidas
}

int validarCalificacion(double calificacion) {
    int parteDecimal;
    // Comprobamos si la calificacion esta dentro del rango permitido
    if (calificacion < 0.0 || calificacion > 10.0) {
        printf("\n\t\t * La calificacion debe estar en el rango de 0.0 a 10.0 *\n");
        return 1; // Indica que hay error
    }
    return 0; // Sin errores
}

int validarNumeroLista(int numeroLista) {
    // Comprobamos si el numero de lista está en el rango permitido
    if (numeroLista < 1 || numeroLista > N) {
        printf("\n\t\t * El numero de lista debe estar en el rango de 1 a %d *\n", N);
        return 1; // Indica que hubo un error
    }
    return 0; // Sin errores
}

void leerUnaCalificacion(int i, int currentPartial, double calif[][3]){
    int errores;
    do{
        printf("Ingrese la ");
        switch (currentPartial){
            case 0: printf("1ra"); break;
            case 1: printf("2da"); break;
            case 2: printf("3ra"); break;
            default: printf("rangoDesconocido"); break;
        }
        printf(" calificacion del alumno %d (del 0.0 al 10.0, se usara 1 solo decimal): ", i+1);
        scanf(" %lf", &calif[i][currentPartial]);
        errores = validarCalificacion(calif[i][currentPartial]); // Llamamos a la función validarCalificacion
        limpiarBuffer();
    } while (errores>0);
    
}

/*
void leerAlumnos(int i, struct alumno alu[]){
    int errores=0;
    do{
        printf("Ingrese el nombre completo del alumno %d (Max. 50 caracteres): ", i+1);
        scanf(" %50[^\n]s", alu[i].nombreComp);
        errores=validarNombre(alu[i].nombreComp);
        limpiarBuffer();
    } while (errores>0);

    do{
        printf("Ingrese la matricula del alumno %d (10 caracteres): ", i+1);
        scanf(" %10[^\n]s", alu[i].matricula);
        errores = validarMatricula(alu[i].matricula); // Llamamos a la función validarMatricula
        if (errores == 0) {
            errores = validarMatriculaRepetida(alu[i].matricula, alu, i); // Llamamos a la función validarMatriculaRepetida
            if (errores == 1) {
                continue; // Volver a solicitar la matrícula
            }
        }
        limpiarBuffer();
    } while (errores>0);

    alu[i].minutosTardados=0;
}

void leerCalifAlumnos(int i, double calif[][3]){
    leerUnaCalificacion(i,0,calif);
    leerUnaCalificacion(i,1,calif);
    calif[i][2]=-1; //Esta calificacion NO se valida, ya que luego se define si acabo o no el examen
}

void horaActual(int *hora, int *minuto) {
    time_t rawtime;
    struct tm *timeinfo;

    time(&rawtime);
    timeinfo = localtime(&rawtime);

    *hora = timeinfo->tm_hour;
    *minuto = timeinfo->tm_min;
}

NODO *insertarAlFinal (int nListaTemp, int horaTemp, int minutoTemp, NODO *nodoRaiz){
    int errores = 0;
    NODO *nuevo = NULL, *aux = NULL;
    if (nodoRaiz == NULL){
        nuevo = crearNodo(nListaTemp, horaTemp, minutoTemp);
        nodoRaiz=nuevo;
        printf("\n\t * ALUMNO AGREGADO AL EXAMEN  *\n");
    }else{
        aux=nodoRaiz;

        if (aux->nLista==nListaTemp) errores++;
        while (aux->siguiente != NULL){
            aux = aux->siguiente;
            if (aux->nLista==nListaTemp) errores++;
        }
        if (errores==0){
            nuevo = crearNodo(nListaTemp, horaTemp, minutoTemp);
            if (nuevo==NULL){
                printf("\n\t\t * Lo sentimos, algo paso.  *\n");
                return NULL;
            }
            aux->siguiente=nuevo;
            printf("\n\t * ALUMNO AGREGADO AL EXAMEN  *\n");
        }else{
            printf("\n\t\t * Se encontro un alumno repetido. No se hara nada.  *\n");
        }
    }
    return nodoRaiz;
}

int leerNumeroDeLista(){
    int errores = 0, nListaT = 0;
    do{
        printf("Ingrese el numero de lista del alumno: ");
        scanf(" %d", &nListaT);
        errores = validarNumeroLista(nListaT); // Llamamos a la función validarNumeroLista
        limpiarBuffer();
    } while (errores>0);
    return nListaT;
}

void alumnoIniciaExamen(NODO **nodoRaiz){
    int horaTemp, minutoTemp, nListaTemp;
    printf("\n ALUMNO INICIO EXAMEN \n");
    nListaTemp = leerNumeroDeLista();
    horaActual(&horaTemp, &minutoTemp); // Llamamos a la funcion horaActual para obtener la hora y minutos actuales
    *nodoRaiz = insertarAlFinal(nListaTemp, horaTemp, minutoTemp, *nodoRaiz);
}

NODO *eliminaNodoAlumno (int info, NODO *nodoRaiz, int *errores, int *horaTemp, int *minutoTemp){
    NODO *aux, *anterior;
    *errores = 1;
    if (nodoRaiz != NULL){
        aux = nodoRaiz;
        if(nodoRaiz->nLista == info){
            nodoRaiz = nodoRaiz->siguiente;
            *errores = 0; //Encontrado
            *horaTemp = aux->marcaDeTiempo.hora;
            *minutoTemp = aux->marcaDeTiempo.minuto;
            free(aux);
        }else{
            aux = nodoRaiz;
            anterior = nodoRaiz;
            while (aux != NULL){
                if(aux->nLista == info) break;
                anterior = aux;
                aux = aux->siguiente;
            }
            if(aux==NULL){
                printf("\n\t\t * Lo sentimos, no se encontro el dato.  *\n");
                return nodoRaiz; // Dato no encontrado
            }
            anterior->siguiente = aux->siguiente;
            errores = 0; //Encontrado
            *horaTemp = aux->marcaDeTiempo.hora;
            *minutoTemp = aux->marcaDeTiempo.minuto;
            free(aux);
        }
    }
    return nodoRaiz;
}

void alumnoTerminaExamen(struct alumno alu[], double calif[][3], NODO **nodoRaiz){
    int errores=0, nListaLeido, horaTempInicio, minutoTempInicio, horaTempFin, minutoTempFin;
    if (*nodoRaiz == NULL){
        printf("\n\t\t * Lo sentimos, NO HAY ALUMNOS HACIENDO EXAMEN  *\n");
    }else{
        printf("\n ALUMNO TERMINO EXAMEN \n");
        nListaLeido = leerNumeroDeLista();
        *nodoRaiz = eliminaNodoAlumno(nListaLeido, *nodoRaiz, &errores, &horaTempInicio, &minutoTempInicio);
        if (errores==0){
            printf("\n\t * Alumno encontrado  *\nSe tomara la hora del sistema para calcular los tiempos\n");
            horaActual(&horaTempFin, &minutoTempFin);
            // Calcular el tiempo tardado en minutos y Asignar el tiempo tardado al alumno correspondiente
            alu[nListaLeido-1].minutosTardados = (horaTempFin - horaTempInicio) * 60 + (minutoTempFin - minutoTempInicio);
            printf("Que calificacion obtuvo? ");
            leerUnaCalificacion(nListaLeido-1,2,calif);
        }
        
    }
}

void resultadosFinales(NODO **nodoRaiz, struct alumno alu[], double calif[][3]) {
    NODO *aux = NULL;
    int i;
    printf("\n\t\t * RESULTADOS FINALES *\n");
    printf("\nN.L\tNombre del Alumno\tMatricula\tTiempo Tardado (minutos)\tCalific 1\tCalif 2\tCalif 3\n");

    while (*nodoRaiz != NULL){
        aux=*nodoRaiz;
        *nodoRaiz = (*nodoRaiz)->siguiente;
        free(aux);
    }
    for (i = 0; i < N; i++){
        if (calif[i][2]==-1) calif[i][2]=5;
        printf("%d\t%s\t%s\t%d\t%.1lf\t%.1lf\t%.1lf\n", i+1, alu[i].nombreComp, alu[i].matricula, alu[i].minutosTardados, calif[i][0], calif[i][1], calif[i][2]);
    }
}

void imprimirMenu() {
    printf("\n\t\t * MENU PRINCIPAL *\n");
    printf("\n\t 1. Agregar alumno que ingreso al examen");
    printf("\n\t 2. Un alumno acaba de terminar su examen");
    printf("\n\t 3. FINALIZAR PROGRAMA\n");
    printf("\n Ingrese la opcion deseada: ");
}

void menuCiclado(struct alumno alu[], double calif[][3], NODO **nodoRaiz) {
    int input;
    do {
        imprimirMenu();
        scanf(" %d", &input);

        switch (input) {
            case 1:
                alumnoIniciaExamen(nodoRaiz);
                break;
            case 2:
                alumnoTerminaExamen(alu, calif, nodoRaiz);
                break;
            case 3:
                resultadosFinales(nodoRaiz, alu, calif); // Llamar a resultadosFinales con los datos de los alumnos
                break;
            default:
                printf("\n\t\t * Input NO valido. Ingrese un numero del 1 al 3 *\n");
                break;
        }
    } while (input != 3);
}
*/
//////
//////


typedef struct nuevoAlu{
    char nombreComp [60];
    int nNom, nApe;
    struct nuevoAlu *siguiente;
} NUEVOALUMNO;

NUEVOALUMNO *crearNodoConDato(char nombreCompTemp[60], int numeroNombre, int numeroApellido){
    NUEVOALUMNO *nuevo = NULL;
    nuevo = (NUEVOALUMNO *) malloc(sizeof(NUEVOALUMNO));
    if(nuevo != NULL){
        strcpy(nuevo->nombreComp, nombreCompTemp);
        nuevo->nNom = numeroNombre;
        nuevo->nApe = numeroApellido;
        nuevo->siguiente = NULL;
    }
    return nuevo;
}

NUEVOALUMNO *insertarOrdenadamente (int numeroNombre,int numeroApellido, NUEVOALUMNO *nodoRaiz, char nombreCompTemp[60]){
    NUEVOALUMNO *nuevo = NULL, *aux = NULL, *anterior = NULL, *next;
    int ordenado = 0;
    nuevo = crearNodoConDato(nombreCompTemp, numeroNombre, numeroApellido);
    if (nuevo==NULL) return NULL;

    if (nodoRaiz == NULL){
        nodoRaiz=nuevo;
    }else{
        aux=nodoRaiz;
        if (nuevo->nNom > aux->nNom){
            while (aux->siguiente != NULL && ordenado == 0){
                if (nuevo->nNom > aux->siguiente->nNom){
                    aux = aux->siguiente;
                    aux->siguiente=nuevo;
                }else{
                    anterior = aux;
                    next = aux->siguiente;
                    anterior->siguiente = nuevo;
                    nuevo->siguiente = next;
                    aux = aux->siguiente;
                    ordenado++;
                }
            }
        }else{
            nuevo->siguiente = aux;
            nodoRaiz = nuevo;
        }
    }
    return nodoRaiz;
}

NUEVOALUMNO *insertarOrdenadamentePorApellido (int numeroNombre, int numeroApellido, NUEVOALUMNO *nodoRaiz, char nombreCompTemp[60]){
    NUEVOALUMNO *nuevo = NULL, *aux = NULL, *anterior = NULL, *next;
    int ordenado = 0;
    nuevo = crearNodoConDato(nombreCompTemp, numeroNombre, numeroApellido);
    if (nuevo==NULL) return NULL;

    if (nodoRaiz == NULL){
        nodoRaiz=nuevo;
    }else{
        aux=nodoRaiz;
        if (nuevo->nApe > aux->nApe){
            while (aux->siguiente != NULL && ordenado == 0){
                if (nuevo->nApe > aux->siguiente->nApe){
                    aux = aux->siguiente;
                    aux->siguiente=nuevo;
                }else{
                    anterior = aux;
                    next = aux->siguiente;
                    anterior->siguiente = nuevo;
                    nuevo->siguiente = next;
                    aux = aux->siguiente;
                    ordenado++;
                }
            }
        }else{
            nuevo->siguiente = aux;
            nodoRaiz = nuevo;
        }
    }
    return nodoRaiz;
}

void leerNuevosAlumnos(int i, NUEVOALUMNO **nodoRaiz, char nombreCompTemp [60]){
    int errores=0, numeroNombre, numeroApellido;
    do{
        printf("Ingrese el nombre completo del alumno %d (Max. 50 caracteres): ", i);
        scanf(" %50[^\n]s", nombreCompTemp);
        errores=validarNombre(nombreCompTemp, &numeroNombre, &numeroApellido);
        limpiarBuffer();
    } while (errores>0);
    *nodoRaiz = insertarOrdenadamente(numeroNombre, numeroApellido, *nodoRaiz, nombreCompTemp);
}

NUEVOALUMNO *eliminaNodo (NUEVOALUMNO *nodoRaiz, int *numeroNombreTemp, int *numeroApellidoTemp, char nombreCompTemp[]){
    NUEVOALUMNO *aux;
    if (nodoRaiz != NULL){
        aux=nodoRaiz;
        nodoRaiz = nodoRaiz->siguiente;
        strcpy(nombreCompTemp, aux->nombreComp);
        *numeroNombreTemp=aux->nNom;
        *numeroApellidoTemp=aux->nApe;
        free(aux);
    }
    return nodoRaiz;
}

void ordenarParEImpar(NUEVOALUMNO **nodoRaiz, NUEVOALUMNO **alumnosPares, NUEVOALUMNO **alumnosImpares){
    int i, numeroNombreTemp, numeroApellidoTemp;
    char nombreCompTemp[60];
    for (i = 1; i <= N; i++){
        *nodoRaiz = eliminaNodo(*nodoRaiz, &numeroNombreTemp, &numeroApellidoTemp, nombreCompTemp);
        if (numeroApellidoTemp%2==0){
            *alumnosPares = insertarOrdenadamentePorApellido(numeroNombreTemp, numeroApellidoTemp, *alumnosPares, nombreCompTemp);
        }else if (numeroApellidoTemp%2!=0){
            *alumnosImpares = insertarOrdenadamentePorApellido(numeroNombreTemp, numeroApellidoTemp, *alumnosImpares, nombreCompTemp);
        }
    }
    
}

void imprimir(int numeroListaLig, NUEVOALUMNO **nodoRaiz){
    NUEVOALUMNO *aux;
    aux=*nodoRaiz;
    printf("\nResultados lista %d:\n", numeroListaLig);
    while (aux!=NULL){
        /*
        if (*nodoRaiz == NULL){
            nuevo = crearNodoALT();
            insertarInfoNodoALT(nLista, numero, &nuevo);
            *nodoRaiz=nuevo;
        }else{
            aux=*nodoRaiz;
            while (aux->siguiente != NULL){
                aux = aux->siguiente;
            }
            nuevo = crearNodoALT();
            insertarInfoNodoALT(nLista, numero, &nuevo);
            aux->siguiente=nuevo;
        }*/
        printf("Nombre: %s\n", aux->nombreComp);
        aux=aux->siguiente;
    }
}

int main(){
    NUEVOALUMNO *nodoRaiz = NULL, *alumnosPares = NULL, *alumnosImpares = NULL;
    int i;
    
    leerNuevosAlumnos(i, &nodoRaiz, "Angel Bautista");
    leerNuevosAlumnos(i, &nodoRaiz, "Jaime Corona");
    leerNuevosAlumnos(i, &nodoRaiz, "Luz Carranza");
    leerNuevosAlumnos(i, &nodoRaiz, "Paola Gazca");
    ordenarParEImpar(&nodoRaiz, &alumnosPares, &alumnosImpares);
    imprimir(1,&nodoRaiz);
    printf("\nLista pares:\n");
    imprimir(2,&alumnosPares);
    printf("\nLista impares:\n");
    imprimir(3,&alumnosImpares);
}