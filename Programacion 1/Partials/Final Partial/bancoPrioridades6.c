...
// Función para generar un identificador único alfanumérico para cada cliente
void generarIdentificador(char identificador[]) {
    static int contador = 0;
    srand(time(NULL));
    char caracteres[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    int longitud = strlen(caracteres);
    for (int i = 0; i < 6; ++i) {
        identificador[i] = caracteres[rand() % longitud];
    }
    identificador[6] = '\0'; // Terminador de cadena
}
