...
short comprobarMesYDia(Fecha *fechaAProcesar, short bis){
    ...
    // Si el mes resultante es mayor que 12
        if ((*fechaAProcesar).mes > 12) {
            (*fechaAProcesar).anio++; //aumenta un año
            (*fechaAProcesar).mes = 1;
        }else if ((*fechaAProcesar).mes < 0){
            (*fechaAProcesar).mes = 1;
        }
    ...
    switch ( (*fechaAProcesar).mes ){
        ...
        case 1: case 3: case 5: case 7: case 8: case 10: case 12:
            if (((*fechaAProcesar).dia) > 31 ){
                (*fechaAProcesar).mes++; // Añade 1 mes
                (*fechaAProcesar).dia = 1;
            }else if (((*fechaAProcesar).dia) < 1){
                (*fechaAProcesar).dia = 1;
            }
        ...
        if ((*fechaAProcesar).mes > 12) {
            (*fechaAProcesar).anio++; //aumenta un año
            (*fechaAProcesar).mes -= 12; //para saber que mes es entonces, resta los meses introducidos con 12 para que se reinicie desde 1 
        }
